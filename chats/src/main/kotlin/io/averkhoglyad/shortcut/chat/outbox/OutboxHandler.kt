...
class MessageOutboxHandlerImpl(
    ...
    override fun handleNext() {
        whileHasPartToProcess { part ->
            part.onEach { sendMessage(it) }
                .mapNotNull { it.id }
                .let { repository.markAsPublishedByIdIsIn(it) }
        }
    }

    private fun whileHasPartToProcess(block: (List<OutboxMessageEntity>) -> Unit) {
    ...
    private fun sendMessage(message: OutboxMessageEntity) {
        ...
        val record = ProducerRecord<String, ByteArray>(topic, message.key, message.body)
...
fun Headers.addAsString(name: String, value: String?): Headers =
    add(name, value?.toByteArray(Charsets.UTF_8) ?: byteArrayOf())
